...
#g(x)=-(1/2)ln(root(2*PI))-(d/2)*ln(|sig|^(1/2))-1/2((X-mu).T)(C.inv)(X-mu))
# #ROC CURVES
...
plt.contour(X11, Y11, np.array(pdf_list[0]), cmap='viridis')
eig,ev=LA.eig(sig)
x=[mu1[0],mu1[0]+500*ev[0][0]]
y=[mu1[1],mu1[1]+500*ev[0][1]]
plt.plot(x,y)
x=[mu1[0],mu1[0]+500*ev[1][0]]
y=[mu1[1],mu1[1]+500*ev[1][1]]
plt.plot(x,y)
# plt.show()
...
x=[mu2[0],mu2[0]+800*ev[1][0]]
y=[mu2[1],mu2[1]+800*ev[1][1]]
plt.plot(x,y)
# plt.show()
...
x=[mu3[0],mu3[0]+800*ev[1][0]]
y=[mu3[1],mu3[1]+800*ev[1][1]]
plt.plot(x,y)
plt.show()
